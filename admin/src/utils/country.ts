...
export const COUNTRY_OPTIONS: CountryOption[] = [
  ...
  { code: 'PS', label: 'Palestinian Territory, Occupied', phone: '970' },
  ...
  { code: 'VG', label: 'Virgin Islands, British', phone: '1-284' },
  { code: 'VI', label: 'Virgin Islands, U.S.', phone: '1-340' },
...
export function getCountryCodeFromPhone(phoneNumber?: string | null): string | null {
  ...
  let bestMatch: { code: string; length: number } | null = null;

  for (const option of COUNTRY_OPTIONS) {
    const normalizedPrefix = option.phone.replace(/\D/g, '');
    ...
    if (normalizedNumber.startsWith(normalizedPrefix)) {
      if (!bestMatch || normalizedPrefix.length > bestMatch.length) {
        bestMatch = { code: option.code, length: normalizedPrefix.length };
      }
    }
  }
